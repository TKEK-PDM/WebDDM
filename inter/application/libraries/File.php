...
class File
{
    ...
    const KO_PATH = 'Work_SharedDir/released/';

    const SJ_PATH = 'Mirror_SharedDir/SJ_SharedDir/released/';

    const ZS_PATH = 'Mirror_SharedDir/ZS_SharedDir/released/';
     ...
     * @param unknown $site            
     */
    public function file_exists($file_name, $site, $org_down)
    {
        $conn = $this->_CI->ftp->connect();
        $file = $this->get_file_path($file_name, $site);

        $size = $this->_CI->ftp->ftp_get_fize($file);
        if($size > 0 && (strtolower(trim(strrchr($file_name, '.'), '.')) == 'dwg' || strtolower(trim(strrchr($file_name, '.'), '.')) == 'pdf')){
            $file = $this->get_file_path($file_name, $site);
        }else {
    ...
    public function get_file_path($file_name, $site, $type = NULL)
    {
        ...
        //ftp_raw('OPTS EUC_KR ON');
        $file_name = mb_convert_encoding(urldecode($file_name), 'UTF-8', 'auto');
...
//            $pdf_file_name = substr($file_name, 0, strlen($file_name) - 3) . 'pdf';
    ...
    public function download_zip(array $file, $zip_name = NULL)
    {
        ...
        log_message("info","LOCAL_PATH : ".$local_path);

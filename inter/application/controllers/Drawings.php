...
class Drawings extends MY_Controller
{
    ...
    public function job_put()
    {
        ...
        if ($where_validation && $data_validation) {
            ...
            if ($this->wd_job->update(array_change_key_case($where, CASE_UPPER),
    ...
    private function assign_supplier($dwg_no, $cg, $user_id)
    {
        ...
        if ($cg['CG'] === null || (trim($cg['CG']) === '')) {

        } else {
            // 들어온 cg가 널이나 공백이 아닐경우에만 삭제 로직을 타도록
            // array_diff를 dwg_sup 테이블에서 도면 번호에 할당된 업체코드가 cg__sup에 cg에 대해 존재하지 않은면 지우도록
            // array_diff() 수정
            // * 잡 조회 또는 도면 조회시
    ...
    public function job_get()
    {
        ...
        if(substr($job1_code,0,1)==1){
            $job1_code = substr($job1_code, 1);
        }
        $job1_code=strtoupper($job1_code);
                ...
                case '3': // Purchasing
                    ...
                    foreach ($job_list as $key => $val) {
                        if ($cg && !in_array($val['COMP_GROUP'], $cg)) {
                            ...
                            $dwg_no = $this->wd_job->get_dwg_no_by_jobcode_and_partcode($job1_code, $job2_code,
                                $val['PART_CODE']);
                            if ($dwg_no) {
    ...
    public function useremail_get()
    {
        $arr_dwg_no = $this->get('dwg_no');
        $first_char = $this->get('first_char');
        
        $search_result = $this->doc->get_useremail_info($arr_dwg_no,$first_char);

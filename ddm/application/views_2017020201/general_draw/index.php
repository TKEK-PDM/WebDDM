...
<div id="page-wrapper">
	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header"><?php echo lang('label_general_drawing');?></h1>
		</div>
	</div>
	<!-- search field -->
			...
			<div class="panel-default">
				...
				<div class="">
					...
					<div id="myTabContent" class="tab-content">
						<div
							class="tab-pane fade <?php if (!$search_count):?>in active<?php endif;?>"
							id="search">
							<form class="ng-pristine ng-valid searchForm keep" role="form"
								...
								<div class="row searchDiv" style="padding-left: 20px">
									<div class="row">
										<div class="col-lg-1">
											<label class="tableLabel"><?php echo lang('label_revision_option');?></label>
										</div>
										<div class="col-lg-10">
											<input name="revision_status" type="radio" value="latest"
												checked="checked"> <?php echo lang('label_latest');?> <input
												name="revision_status" id="revision_status" type="radio"
												value="all"> <?php echo lang('label_all_revision');?>
										</div>
									</div>
									...
									<div class="row buttonSpace">
										<div class="col-lg-2">
											<label for="name"><?php echo lang('label_input_drawing_numbers');?></label>
											<textarea id="dwg_no" name="dwg_no" required="required"
												title="<?php echo lang('message_field_required'); ?>"
												class="form-control" rows="10"><?php echo $request_dwg_str;?></textarea>
										</div>
									</div>
								</div>
							</form>
							...
							<div class="panel panel-default">
								...
								<div class="panel-body table_panel">
									<form action="<?php echo site_url('download/download_zip');?>"
										...
										<?php foreach ($data as $key=>$val):?>
											<tr>
													...
													<?php if ($this->session->user_role == 1):?>
														...
														onclick="check_file_exist(<?php echo "'".$val->FILE_NAME."','".$val->SITE_NAME,"','".$val->REVISION."','".$val->OBJECT_ID."','".$val->CLASS_ID."','".$val->TDM_ID."'";?>)"><?php echo $val->FILE_NAME;?> </a>

    													<?php else: ?><?php echo $val->FILE_NAME;?>
    													<?php endif;?>
    												<?php endif;?>
													...
													<td><?php echo $val->downdate;?></td>
													<td><?php echo $val->downtime;?></td>
													<td id="<?php echo 'pdf_'. $val->OBJECT_ID;?>"><input
														type="hidden"
										...
										<button class="btn btn-outline btn-blue buttonSpace"
											type="button" id="check_file_exist"
						...
						<div
							...
							<form class="form-horizontal" role="form"
								...
								<div class="form-group searchDiv5">
									<label class="tableLabel col-lg-2 smallLabel"><?php echo lang('label_supplier_code');?></label>
									<div class="col-lg-4">
										<input class="form-control normalInput" type="text"
											id="sup_code" name="sup_code" value=""
											onblur="get_sup_name(this)">
									</div>
									<label class="tableLabel col-lg-2 smallLabel"><?php echo lang('label_supplier_name');?></label>
									<div class="col-lg-4">
										<input class="form-control normalInput" type="text"
								...
								<div class="form-group col-lg-12 table_panel">
									<div class="panel panel-default">
										<div class="panel-heading panel_blue">
											<b><?php echo lang('label_general_drawing_list');?>(<?php echo count($data);?>)</b>
										</div>
											...
											<table
        										...
        										<tr>
														<td><input type="checkBox" id="" class="mp_info"
															...
															value="<?php echo $val->TDM_ID;?>"></td>
														<td><?php echo $key+1; ?></td>
														<td><?php echo $val->CLASS_NAME;?><input type="hidden"
															name="class_id[]" value="<?php echo $val->CLASS_ID;?>"></td>
														<td><a
														...
														<td><a href="javascript:void(0);"
															onclick="check_file_exist(<?php echo "'".$val->FILE_NAME."','".$val->SITE_NAME,"','".$val->REVISION."','".$val->OBJECT_ID."','".$val->CLASS_ID."','".$val->TDM_ID."'";?>)"><?php echo $val->FILE_NAME;?> </a>
															</a> <input type="hidden" name="exist_file_name[]"
															value="<?php echo $val->FILE_NAME?>"></td>
														...
														<td><span class="firstSupCode"><?php echo $val->SUP_CODE;?></span>
															<span class="supNum">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
		...
		$("#check_file_exist").click(function(){
	    	...
	    	$.ajax({
	      		...
	      		async: true,
	      		cache: false,
	      		url: "<?php echo site_url('general_draw/check_files'); ?>",
	      		dataType:"json",
	      		data:$("#check_files").serialize(),
	          		...
	          		$.each(msg, function(i,n){
						...
						if(arrResult[0] =="Y"){
							var link = "<a href='javascript:void(0);' "+
									"onclick=\"check_file_exist('"+file_name+"','"+site_name+"','"+revision+"','"+object_id+"','"+class_id+"','"+tdm_id+"')\">"
									+arrResult[0]
									+"</a>"
							t.cell("#"+i).data(link);
						}
  ...
  //send email event
  function send_email(){
	  window.location.href = "mailto:rira.seo@tkeap.com;jeonghyun.nam@tkeap.com?cc=seonuk.lee@tkeap.com&subject=[Web DDM] Missing Drawing No.&body=Requester ID:<?php echo $this->session->user_login;?>%0ARequester Name:<?php echo $this->session->user_display_name;?>%0A%0ABelow Dwg No does not exist%0A%0A<?php echo $email_dwg_email;?>";
